...
R中的逻辑运算符号有两组，`|`{:.language-r}或者`||`{:.language-r}（`&`{:.language-r}或者`&&`{:.language-r}），看起来觉得有些多余。其实，这是为了R独特的“**向量运算**”考虑的。`|`{:.language-r}或者`&`{:.language-r}首先考虑返回一个向量，而`||`{:.language-r}和`&&`{:.language-r}则只返回第一个比较结果。比如：
...
同时，配合使用`droplevels()`（去除多余level）、`relevel()`（以某个level为基准）和`ordered(.., levels)`（调整level顺序）等操作factor的level。
