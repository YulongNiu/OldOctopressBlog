...
> * f：字符串，泛函名称。
> 
> * signature：字符向量，表示`definition`函数变量对应的类名。两个特殊类`"missing"`和`"ANY"`，分别表示对应的变量“不能出现在方法调用中”和“可以是任何类”。如果一个变量没有被指定，则默认为`"ANY"`。个人推测，这个设定是为了满足“泛函和扩展函数形式一致”的设计。
>
...
方法和泛函的关系非常密切，某一个方法必须建立在一个特定泛函下。对于一个对象，可以用同一个一个方法处理多个不同情况。
...
* `findMethods('genfun')`：查看某个泛函的具体信息。
