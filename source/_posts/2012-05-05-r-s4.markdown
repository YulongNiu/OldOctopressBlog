...
`setClass()`的`contains`参数规定了继承的父类，很直接的继承关系。使用`as()`（不改变原始对象的值）和`as() <- value`进行转换。
...
* `as(objSon, 'classFather') <- objSon`等同于`as(objSon, 'classFather')`。

* `as(objFather, 'classSon') <- objFather`等同于`as(objFather, 'classSon')`。

* `as(objSon, 'classFather') <- objFather`父类slots的值“遗传”给子类。
