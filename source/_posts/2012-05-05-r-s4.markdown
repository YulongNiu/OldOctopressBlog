...
> * contains：父类名，表示继承关系。
>
> * prototype：带名字的列表或`prototype()`，设定默认值。不建议添加。设定时，要结合`validity`定义，不要把默认值设定得无法通过检查。
>
...
创建对象时，会有两种检查机制。1. 自动检查每个slot赋值类型是否正确，这种检查会一直被子类和slot含有该类的类（简称“slot含有类”）继承。2. 如果设定了`validity`，这种检查将一直被子类继承，但是slot含有类不会去检查slot赋值是否正确。对于这种情况，将`validObject(object, test = FALSE, complete = FALSE)`函数的`complete`参数设定为`TRUE`，可以检查出问题。
