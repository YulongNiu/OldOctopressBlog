...
重置`[`、`[[`和`$`获取对象。重置`[<-`、`[[<-`和`$<-`修改对象（改变原始对象），建议使用`validObject()`检查修改后的对象。使用`setGroupGeneric()`定义新的泛函组，使用`callGeneric()`调用泛函组，比如：
...
S4允许将一类方法定义为“泛函组（group generic functions）”。已经定义的泛函组，比如`ops`，通过`?S4groupGeneric`查看。
