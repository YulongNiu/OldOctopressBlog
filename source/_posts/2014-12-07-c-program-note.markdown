...
<span style="color: green">**C99**</span>中增加了`ll`或者`LL`后缀，强制`long long int`型整数，可以与`u`或`U`连用。**避免无符号和有符号整数混用**。

* 强制编辑器处理常量为单精度浮点数，`11.3f`，`11.3`会被认为是`double`型；强制为双精度，`11.3L`或者`11.3l`。
