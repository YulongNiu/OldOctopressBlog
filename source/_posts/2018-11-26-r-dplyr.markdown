...
* `filter()`、`arrange()`、`mutate()`、`select()`、`transmute()`、`rename()`和`summerise()`：有`_all()`、`_at()`和`_if()`操作，分别表示对全部列、指定列（序号`1:4`和`-1:-2`）和满足条件的列操作。

> `_at()`使用`contains()`等函数时，`.var`参数需要处理为`.var = vars(contains())`的形式。

> 参数中的`.funs`操作是一个匿名函数，例如`.funs = funs(ifelse(x, ., NA))`，`.`表示对应的列。同时，`.funs`可以有多个操作(同时赋值列名)，比如`.funs = funs(repNA = ifelse(x, ., NA), repEmp = replace_na(., ''))`。在`.funs`中使用`expr(.)`引用列名。
